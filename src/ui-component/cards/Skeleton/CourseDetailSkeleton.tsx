...
const CourseDetailSkeleton: React.FC = () => {
  ...
  const isMobile = useMediaQuery(theme.breakpoints.down('md')); // Kiểm tra xem có phải mobile không

  return (
    <Box sx={{ bgcolor: theme.palette.background.paper, minHeight: '100vh', color: 'text.primary' }}>
      <Stack
        direction={isMobile ? 'column' : 'row'} // Thay đổi hướng layout trên mobile
        spacing={4}
        sx={{ p: 3 }}
      >
        {/* Left Content */}
        <Stack spacing={3} sx={{ flex: 1 }}>
          {/* Course Title & Rating */}
          <Box>
            <Skeleton variant="text" width={isMobile ? '80%' : '60%'} height={40} sx={{ mb: 1 }} />
          ...
          {/* Course Description */}
          <Skeleton variant="text" width={isMobile ? '100%' : '90%'} />
          <Skeleton variant="text" width={isMobile ? '60%' : '40%'} />

          {/* Course Content */}
          <Box sx={{ mt: 4 }}>
            <Skeleton variant="text" width={isMobile ? '60%' : '40%'} height={32} sx={{ mb: 2 }} />
            <Skeleton variant="text" width={isMobile ? '80%' : '60%'} sx={{ mb: 2 }} />
            ...
            {[...Array(4)].map((_, index) => (
              ...
              >
                ...
                <Skeleton variant="text" width={isMobile ? '30%' : '20%'} />
              </Stack>
            ))}
          </Box>
        </Stack>
        ...
        {/* Right Content */}
        {!isMobile && ( // Ẩn right content trên mobile
          <Box sx={{ width: 400 }}>
