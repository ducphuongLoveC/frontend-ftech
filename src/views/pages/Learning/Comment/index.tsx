...
export default function Comment() {
  ...
  return (
    <Box maxWidth="md" margin="auto" padding={2}>
      ...
      <Stack spacing={3}>
        {comments.length ? comments.map((c: CommentData) => <CommentItem user={user} key={c._id} comment={c} parent_id={c._id} deep={1} />) : <Box textAlign={'center'}>Không có nội dung comment</Box>}
      </Stack>
    </Box>
  )
}
...
function CommentItem({ user, fatherCommentUser = null, comment, deep = 1 }: CommentItemProps) {
  ...
  return (
    <Box className="comments" data-id={comment._id}>
      <Box display="flex">
        ...
        <Box flexGrow={1}>
          ...
          <Typography variant="body2" paragraph sx={{ mt: 0.5, color: 'text.primary' }}>
            <Box display={'flex'} alignItems="center">
              {fatherCommentUser && (
                <MUILink mr={1} target="_blank" to={path.client.profile(`?id=${fatherCommentUser._id}`)} component={Link}>
      ...
      {isReplying && (
        <Box ml={7} mt={2}>
          <CommentInput mutation={mutation} user={user} onSubmit={handleReply} placeholder="Add a reply..." buttonText="Trả lời" />
        </Box>
      )}
        ...
        <Box ml={deep === 1 ? 7 : 0} mt={2}>
          <Stack spacing={2}>
            {comment.replies.map((reply, index) => (
              <CommentItem parent_id={comment._id} user={user} fatherCommentUser={comment.user} key={index} comment={reply} deep={deep + 1} />
            ))}
          </Stack>
        </Box>
      )}
