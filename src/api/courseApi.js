...
export const getSingleCourseById = async (id) => {
    try {
...
export const getCourseSearch = async (search) => {
    const res = await axiosInstance.get(`api/courses?search=${search}`);
    return res.data;
};
export const getCourseStatistics = async (id, params) => {
...
export const getCourseFull = async (id) => {
...
export const newCourse = async (data) => {
    ...
    if (Array.isArray(data.learning_path_ids)) {
        console.log('check');
        data.learning_path_ids.forEach((id) => formData.append('learning_path_ids[]', id));
    }
        ...
        module.resources.forEach((resource, resourceIndex) => {
            ...
            switch (resource.resource_type) {
                case 'Video':
                    ...
                    break;
                case 'Question':
                    resource.questions.forEach((question, questionIndex) => {
                        // Append question text
                        formData.append(`modules[${moduleIndex}][resources][${resourceIndex}][questions][${questionIndex}][question]`, question.question);
                        // Append options
                        Object.keys(question.options).forEach((optionKey) => {
                            formData.append(`modules[${moduleIndex}][resources][${resourceIndex}][questions][${questionIndex}][options][${optionKey}]`, question.options[optionKey]);
                        });
                        // Append the correct answer
                        formData.append(`modules[${moduleIndex}][resources][${resourceIndex}][questions][${questionIndex}][correctAnswer]`, question.correctAnswer // You need to ensure that this field is correctly mapped in your schema
                        );
                        formData.append(`modules[${moduleIndex}][resources][${resourceIndex}][questions][${questionIndex}][hint]`, question.hint);
                    });
                    break;
    ...
    data.modules.forEach((module, moduleIndex) => {
        ...
        module.resources.forEach((resource, resourceIndex) => {
            ...
            switch (resource.resource_type) {
                ...
                case 'Question':
                    resource.questions.forEach((question, questionIndex) => {
                        formData.append(`modules[${moduleIndex}][resources][${resourceIndex}][questions][${questionIndex}][_id]`, question._id || '');
                        // Append question text
                        formData.append(`modules[${moduleIndex}][resources][${resourceIndex}][questions][${questionIndex}][question]`, question.question);
                        // Append options
                        Object.keys(question.options).forEach((optionKey) => {
