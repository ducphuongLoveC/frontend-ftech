...
//   }
// };
export const getNotes = async (resource_id, user_id, queries) => {
    const queryObj = {};
    queries.forEach((q) => {
    ...
    const queryString = Object.entries(queryObj)
        ...
        .join('&');
    try {
        const { data } = await axiosInstance.get(`api/note?resource_id=${resource_id}&user_id=${user_id}${queryString ? `&${queryString}` : ''}`);
        return data.notes;
    }
    catch (error) {
        throw error;
