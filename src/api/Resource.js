...
export const getResource = async (course_id, user_id, id) => {
    const res = await axiosInstance.get(`api/resource/progress/${course_id}/${user_id}/${id}`);
    return res.data;
};
export const getNameModuleById = async (id) => {
...
//   return res.data;
// };
export const getAdjacentResourceId = async (id, direction, user_id) => {
    const res = await axiosInstance.get(`api/resource/${id}/adjacent/id?direction=${direction}&user_id=${user_id}`);
    return res.data;
};
export const getFirstResourceId = async (course_id) => {
    try {
